...
“像幻灯片”是一个可以量化的说法：统计相邻帧差异低于阈值的比例，即“冻结帧占比”。旧引擎在
文字型模板上实测 56–80%；现在四模板混合是 0%，全部测试工程都在 15% 以下，`verify` 会卡这条线。

做到这一点靠的是四件事，都是结构性的，不是加特效：

| 机制 | 解决什么 |
| --- | --- |
| `drift` 持续漂移 | 每个元素在关键帧之上永远缓慢游走（三个不可通约频率、各自相位），所以不存在两帧完全相同的画面 |
| `Anim.Path` 相机样条 | 相机不再每个节拍从静止重新起步；整镜头是一条连续轨迹 |
| `relayout` 重组 | 文字块与主体换边，同一批素材换一种构图 |
| `restage` 硬切重构图 | 在同一个镜头内部切到新的取景，并有擦除过渡 |

`relayout` / `restage` 由节拍表在独立时钟上强制发生（`PAGE_DWELL` = 1.8–3.0 秒，随节奏变化）,
这就是直接回答“同一页停留太久”。相机位移按图层深度分配：背景和主体承担运动，文字层几乎不跟
着走——因为这些版式本来就是贴边的，文字跟着相机会切到自己的边。

完整原理、实测数据和写新模板的清单见 [`references/animation.md`](references/animation.md)。
...
**关键杠杆：降低长段落的帧率，而不是把它变成一张静止图。**

```jsonc
{ "id": "ch03", "template": "caption", "duration": 24.0,
  "data": { "anim_fps": 12, "title": "第三步：交付", "caption": "把结论写成一句话。" },
  "assets": { "subject": "assets/fig03.png" } }
```

`"anim_fps": 12` 按 12 帧渲染，再由剪辑步骤把帧复制回项目帧率。**成本随帧率线性下降，但画面仍然在动。**
也可以在项目级一次设定:`"render": { "anim_fps": 12 }`。

`"still": true` 仍然存在，只渲染一帧，成本与时长无关——但它产出的确实是静止画面。旧版长视频
基本都靠它，所以整支片子看着像幻灯片。现在它只适合真正的标题卡和尾板；如果你给一大段标了
`still`，`verify` 的 `alive` 检查会把那部分时长计入冻结预算，并如实报出占比。

实测：18 秒的旁白段从 `still` 改成 `anim_fps: 12` 后，冻结帧占比从 35% 降到 2%，时长不变。
...
| `content` | 全黑、素材没加载进来 |
| `alive` | 冻结帧占比超过“15% + 你声明的静止时长”；也就是“还是像 PPT” |
| `fade_in` / `fade_out` | 淡入淡出根本没生效（时长算错时很常见） |
| `ink` | 演员从未渲染（整屏没有亮像素——旧版四个模板就是这样整场空白的） |
...
渲染器还会对每个段落做版式审计：五个时刻采样，按元素边界判断出画，并报告从未可见的文字、
重叠的文字块、以及回退成衬线体的页面。这些警告在 `run` 时直接打出来，不必等验收。
