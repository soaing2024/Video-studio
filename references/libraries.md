...
写一支片子要四类外部资源：**图标库**（语义符号）、**动效库**（怎么动）、**素材库**（画面从哪来）、**音效库**（声音从哪来）。
...
放进 `assets/lib/<name>/`。一条命令就能做完：

```powershell
python vs.py libs                      # 看已经落盘了哪些、还能装哪些
python vs.py libs --install lottie     # 内部就是 npm pack + 解包 + 留 dist 与 LICENSE
```
...
图标是“文字说不清、画图又太重”时的第三选项，但用错会立刻变成装饰噪音。
判断标准就一条：**它有没有承担信息**。“播放”“外链”“第三章”有意义；一排彩色 icon 当花纹没有。

| 库 | 授权 | 发布产物 | 能不能直接进场景 |
| --- | --- | --- | --- |
| **Lucide** 1.47 · **已落盘** | ISC | `dist/umd/lucide.min.js`（442KB，2108 个图标） | ✅ `"libs": ["lucide"]` + `Scene.icon()` |
| Tabler Icons 3.47 | MIT | `icons.json`（1.96MB，含 SVG 路径） | 可以，但要自己写个小包装把条目转成 `<svg>` |
| Phosphor Icons 2.1 | MIT | webfont（woff2 + css） | 要先 base64 内联字体，否则 `file://` 下路径会断 |
| Simple Icons 16.31 | CC0-1.0 | `index.js`（5.2MB）逐条含 path；`data/*.json` 只有元数据 | 体积大，建议先抽子集再落盘 |
| Feather | MIT | SVG / JS | 已被 Lucide 接替，直接用 Lucide |
| Material Symbols | Apache-2.0 | 可变字体 | 同字体问题；语义偏 UI，不太适合叙事片 |
| Heroicons | MIT | SVG（React/Vue 优先） | 可用，但要自己准备数据 |

**本流水线只落盘 Lucide**，理由很具体：它是唯一“一个 UMD 文件 + 完整图标数据 + 宽松授权 +
不依赖字体”的组合。其余几个要么是 ESM-only、要么只发字体、要么单文件太大。要换风格就按
第 1 节的方法 vendor 进来，并在 `scripts/lib/libs.py` 的注册表里登记。

用法（先在工程里写 `"libs": ["lucide"]`）：

```js
const arrow = Scene.icon("arrow-right", { size: 64, color: "#e0455f", strokeWidth: 2 });
layer.appendChild(arrow);
arrow.style.transform = `translateX(${(t * 120).toFixed(1)}px)`;   // 动效自己算，仍然是 t 的纯函数
```

- 名字两种写法都行：`"arrow-right"` 与 `"ArrowRight"`；`Scene.iconNames()` 列出全部 2108 个。
- 默认 `stroke: currentColor`，跟着父元素颜色走；要独立颜色就传 `color`。
- 图标是 DOM 而不是字体：不受字体回退影响，也不会在渲染时联网。
- **不要**把图标当项目符号或花纹用。一屏最多一个语义图标，而且它要落在安全区内。
